  ...
  export default function SystemConfigPage() {
    ...
    const saveIntegrationSettingsMutation = useMutation({ // This one might be intended for a more generic integration settings save
      mutationFn: async (settings: any) => {
        const res = await fetch(API_ROUTES.SYSTEM_SETTINGS, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ lms: { value: settings } }), // Assuming it saves all integration settings under 'lms' key
        });
        if (!res.ok) throw new Error("Failed to save");
        return res.json();
      },
    ...
    // Save LMS integration settings (specific mutation for /api/admin/lms/credentials)
    ...
    const handleIntegrationSettingsSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      // ===== RESOLVED CONFLICT: Using logic from 'main' branch =====
